	...
	(async () => {
		try {
			...
			for (i = 0; i < len; i++) {
				...
				if (resp.rows[i].item_type === 1) {                                       // 图片
					album_items = album_items + '<li><a href="##" onclick="set_picture_as_avatar(' + resp.rows[i].item_id + ', ' + resp.rows[i].origin_length + ');">' + $("#set_as_avatar").html() + '</a></li>';
				}
...
function set_picture_as_avatar(item_id, origin_length)
{
	$("#album_item_dropdown_" + item_id).dropdown('close');

	if (origin_length > 819200) { // 头像图片必须小于等于800KB
		if (get_cookie('i18n_lang') === "zh") alert("错误：头像图片不能超过800KB。");
		else                                  alert("Error: The avatar image cannot exceed 800KB.");
		return;
	}
		...
		(async () => {
			try {
				...
				if (user_avatar_map.has(user)) {
					//console.log("user avatar get");
					resolve( user_avatar_map.get(user) );
				}
				else {
					//console.log("user avatar no get");
					...
					if ( resp.rows.length === 1 && resp.rows[0].user === user && resp.rows[0].avatar_album_item_id > 0 ) {
						...
						if ( resp.rows.length === 1 && resp.rows[0].item_id === img_id && resp.rows[0].item_type === 1 && resp.rows[0].origin_length <= 819200 ) {  // 头像图片必须小于等于800KB
							let memo        = '';
							let next_hash   = '';
							let content     = '';
							let transaction = null;
...
function content_album_items_load_file(item_id, item_type, storage_location, origin_head_hash, origin_sha3_hash)
{
	$("#my_modal_loading").modal('open');
	(async () => {
		try {
			...
			if (item_type === 1) {                                       // 图片
				...
				$(".div_content_album_item_" + item_id).html('<video src="' + file_content + '" controls style="width:auto; height:auto; max-width:100%; max-height:100%;">HTML5 Video is required.</video>');
			}
			else if (item_type === 3) {                                  // 音频
				$(".div_content_album_item_" + item_id).html('<audio src="' + file_content + '" controls>HTML5 Audio is required.</audio>');
			}
			else if (item_type === 5) {                                  // 其他
				$(".div_content_album_item_"          + item_id).html('<div><input type="hidden" class="content_album_item_hidden_input_' + item_id + '" value="" /></div>' + file_content.slice(0, album_item_data_preview_length) + '......');
				$(".content_album_item_hidden_input_" + item_id).val(file_content);
			}
			else {
			}
			$("#my_modal_loading").modal('close');
